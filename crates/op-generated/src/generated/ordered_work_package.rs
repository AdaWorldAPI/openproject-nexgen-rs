// @generated by ogar_render_askama (OGAR) — do not edit by hand.




//! `OrderedWorkPackage` — canonical class generated from `ogar_vocab::orderedworkpackage()`.
//! Fields are the ClassView × FieldMask projection; methods are the OGAR
//! `ActionDef` DO-arm (behaviour is Rust, never SurrealQL DDL).
//! DO NOT EDIT BY HAND. Re-render via `ogar-render-askama`.

/// Canonical concept name as in the OGAR codebook.
pub const CANONICAL_CONCEPT: &str = "orderedworkpackage";

#[derive(Debug, Clone, Default, PartialEq)]
pub struct OrderedWorkPackage {
    /// attribute `id`.
    pub id: i64,
    /// attribute `position`.
    pub position: i64,
    /// belongs_to `query`.
    pub query: Option<u64>,
    /// belongs_to `work_package`.
    pub work_package: Option<u64>,
}

impl OrderedWorkPackage {

    /// Struct-of-methods constructor over the ClassView × FieldMask field set.
    pub fn new(id: i64, position: i64, query: Option<u64>, work_package: Option<u64>) -> Self {
        Self { id, position, query, work_package }
    }
}