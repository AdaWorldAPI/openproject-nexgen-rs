...
> premise was falsified.** `git ls-remote`/`git clone` of the AdaWorldAPI repos
> succeed over the proxy. So **both lance-graph and OGAR are now plain Cargo git
> deps** (see the retired table), and D1/D2/D4 — which existed only to make an
> offline vendored slice resolve — are retired. **ruff is the last vendored
> mirror**, and only because it carries the local D3 patch; once R1 lands
> upstream, ruff becomes a git dep too and `vendor/` + `vendor-sync.sh` retire
> entirely.
...
| — | **OGAR vendoring (whole `vendor/AdaWorldAPI-OGAR` mirror, ~12k LOC)** | Same false-premise vendoring; carried no local *source* deviations (only the D2/D4 Cargo.toml redirect) | 2026-07-02 | **2026-07-05** | **Un-vendored the same way.** The 3 consumed crates (`ogar-vocab`, `ogar-render-askama`, `ogar-class-view`) depend only on each other + `lance-graph-contract` (git, unifies) + crates.io `askama` — **no ruff dep** — so no collision with the still-vendored ruff. op-nexgen's 5 consumer lines now use `{ git = ".../OGAR", branch = "main" }`; Cargo lock-pins `76dcb27` as one unified source. |
