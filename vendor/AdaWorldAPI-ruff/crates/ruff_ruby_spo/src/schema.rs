...
use ruff_spo_triplet::{Field, ModelGraph};
...
fn symbol_token(part: &str) -> Option<&str> {
    part.trim().strip_prefix(':').map(|s| s.trim_end())
}
...
pub(crate) fn model_name_for_table(table: &str) -> String {
    ...
    const IRREGULAR: &[(&str, &str)] = &[("news", "news"), ("meeting_agenda_item_series", "meeting_agenda_item_series")];
...
"#;
    ...
    fn parses_the_dsl_forms() {
        ...
        assert_eq!(by_name("created_at").field_type.as_deref(), Some("datetime"));
        assert_eq!(by_name("created_at").not_null, None);
        // belongs_to alias.
        assert_eq!(by_name("responsible_id").field_type.as_deref(), Some("bigint"));
        // Polymorphic pair — the PolyRef substrate declaring itself.
        assert_eq!(by_name("reactable_id").not_null, Some(true));
        assert_eq!(by_name("reactable_type").field_type.as_deref(), Some("string"));
    ...
    fn openproject_corpus_schema_gate() {
        ...
        assert!(report.tables_seen >= 90, "expected ~99 baseline tables, saw {}", report.tables_seen);
        assert!(report.tables_matched >= 50, "matched only {}", report.tables_matched);
        eprintln!(
        ...
        assert_eq!(by_name("done_ratio").not_null, None, "unset ≠ 0% — the oracle-diff bug");
